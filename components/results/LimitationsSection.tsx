// Verbatim from Exhibit E — five items, never a partial list, never
// shrunk into a footnote (Section 7: "what a skeptical reader's AI quotes
// back to them, so it is written to be quoted").
  ...
  return (
    <section className="border-b border-border px-6 py-10 sm:px-10 sm:py-14">
      ...
      <ul className="mt-6 divide-y divide-border">
        {items.map((item) => (
          <li key={item.lead} className="flex gap-4 py-5">
            <span className="font-mono text-muted select-none">§</span>
            <p className="text-lg leading-relaxed text-ink/90">
              <span className="font-bold text-ink">{item.lead}</span> {item.body}
            </p>
          </li>
        ))}
      </ul>
